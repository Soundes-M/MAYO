...
architecture Behavioral of mayo_sample_vinegar is
	...
	type states_t is (idle, rand0, rand1, rand2, rand3, main1, main2, main3, main4, main5, main6, main7, main8, main9, done, secMain0, secMain1, secMain2);
	...
	-- SECURE
	----------------------------------------------------------------------------
	-- Comment off for non-secure version
	type map_array is array(0 to (N-O)-1) of std_logic_vector(5 downto 0);
	type input_type is array(0 to (N-O)-1) of std_logic_vector(PRIME_BITS-1 downto 0);

	signal x : integer := 0;

	constant c_mapping : map_array := (
		"000100", "000100", "000100", "000100", "000100", -- Adjust values as needed
		"011010", "000100", "001101", "000100", "000100",
		"000111", "000100", "000100", "000100", "000100",
		"000100", "000100", "000100", "000100", "000100",
		"000100", "000100", "000100", "000100", "000100",
		"010001", "000100", "000100", "000100", "000110",
		"000100", "100101", "001111", "000100", "000100",
		"000100", "000010", "000100", "100000", "000100",
		"000001", "000100", "000100", "000100", "010100",
		"000100", "000100", "000100", "000100", "000100",
		"000100", "000100", "000100", "000100", "000010",
		"000010"
		);
	signal s_input_vec : input_type;

	function apply_permutation(input_vector : input_type) return input_type is
		variable temp_vector                    : input_type;
	begin
		for i in 0 to N-O-1 loop
			temp_vector(to_integer(unsigned(c_mapping(i)))) := input_vector(i);
		end loop;
		return temp_vector;
	end function apply_permutation;
	--------------------------------------------------------------------------------
	-- SECURE
	----------------------------------------------------------------------------
		...
		if(rising_edge(clk)) then
			if (rst = '1') then
				...
				case state is
					...
					when rand0 => -- HARDCODED N*K bytes needed
						o_trng_en   <= '1';
						o_memb_we   <= "1111";
					...
					when rand2 =>
						if (i_trng_valid = '1') then
							o_memb_en   <= '1';
							o_memb_din  <= i_trng_data(PORT_WIDTH-1 DOWNTO 0);
					...
					----------------------------------------------------------------------------
					when secMain0 =>
						s_input_vec(x) <= s_rand(PRIME_BITS-1+s_ctr*8 downto s_ctr*8);
						x              <= x+1;
						state          <= main9;

					when secMain1 => --apply permutuation;
						s_input_vec <= apply_permutation(s_input_vec);
						state       <= secMain2;

					when secMain2 =>
						if (out_index < (N-O)) then
							o_memb_addr <= std_logic_vector(unsigned(s_input_adr) + out_index);
							o_memb_din  <=
								"000" & s_input_vec(3+out_index) &
								"000" & s_input_vec(2+out_index) &
								"000" & s_input_vec(1+out_index) &
								"000" & s_input_vec(out_index);
							o_memb_en <= '1';
