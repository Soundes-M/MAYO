...
module SHA_SHAKE_wrapper(clk, rst, shake_intermediate_rst, shake_next_extract, rate_type, mlen,
    ...
    always @(state or buffer_full or rate_full or mlen_g8)begin
        case(state)
            4'd0: begin
                start<=1'b1;
                rst_keccak<=0;
            ...
            4'd1: begin
                start<=1'b0;
                rst_keccak<=1; /*inc_rd_address<=1;*/
                din_valid<=1'b1;
                last_block<=1'b0;
                if(mlen_g8==1'b0 || rate_full==1'b1)
                    dec_mlen<=0;
                else
                    dec_mlen<=1;
                if(rate_full)
    ...
    always @(state or buffer_full or last_rate_byte or sample_dout or olen_is0
                 or shake_intermediate_rst or shake_next_extract)begin
        case(state)
            4'd0:
                nextstate <= 4'd10;
            4'd10:
