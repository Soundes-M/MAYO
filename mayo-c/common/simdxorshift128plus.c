...
void xorshift_expand(const unsigned char* seed, unsigned char* out, long long out_len){
	uint32_t i = 0;
  	avx_xorshift128plus_key_t mykey;
  	avx_xorshift128plus_init(*((uint64_t*) seed), *((uint64_t*) (seed+8)), &mykey);

  	const uint32_t block = sizeof(__m256i) / sizeof(unsigned char); // 8
  	while (i + block <= out_len) {
    	_mm256_storeu_si256((__m256i *)(out + i), avx_xorshift128plus(&mykey));
    	i += block;
  	}
  	if (i != out_len) {
    	unsigned char buffer[sizeof(__m256i) / sizeof(unsigned char)];
    	_mm256_storeu_si256((__m256i *)buffer, avx_xorshift128plus(&mykey));
    	memcpy(out + i, buffer, sizeof(unsigned char) * (out_len - i));
  	}
}
